...
function employeeDetals(employeeID, fullName, department,
    ...
    employeeDetals.allEmployyes.push(this);
...
const calculateSalary = (min, max) => {
    ...
    salaryAfterTax = salaryBeforeTax - (salaryBeforeTax * 0.075);
...
function saveToLocalStorage() {
    let stringifedData = JSON.stringify(employeeDetals.allEmployyes);
    localStorage.setItem("card", stringifedData);
}
function getData() {
   ...
   if (parseData != null) {
       ...
       for (let i = 0; i <parseData.length; i++) {
            ...
            new employeeDetals(parseData[i].name, parseData[i].department, parseData[i].imageURL, parseData[i].level);
        }
    }
...
function renderAll() {
    for (let i = 0; i < employeeDetals.allEmployyes.length; i++) {
        employeeDetals.allEmployyes[i].render();
    }
